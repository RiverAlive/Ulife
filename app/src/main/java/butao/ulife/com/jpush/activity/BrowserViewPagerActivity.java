...
import butao.ulife.com.jpush.chatting.photoview.PhotoView;
import butao.ulife.com.jpush.chatting.utils.BitmapLoader;
import butao.ulife.com.jpush.chatting.utils.HandleResponseCode;
import butao.ulife.com.jpush.tools.NativeImageLoader;
import butao.ulife.com.jpush.view.ImgBrowserViewPager;
...
//用于浏览图片
public class BrowserViewPagerActivity extends BaseIMActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(CApplication.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(CApplication.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(CApplication.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(CApplication.POSITION, 0);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(CApplication.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(CApplication.RESULT_CODE_SELECT_PICTURE, intent);
                    mBitmap.recycle();
                    mBitmap = null;
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(CApplication.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(CApplication.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(CApplication.MsgIDs, activity.mMsgIds);
                        activity.setResult(CApplication.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
