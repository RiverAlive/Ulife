package butao.ulife.com.jpush.receiver;
...
public class NotificationClickEventReceiver {
    ...
    public void onEvent(NotificationClickEvent notificationClickEvent) {
        ...
        if (msg != null) {
            ...
            Conversation conv;
            Intent notificationIntent =null;//= new Intent(mContext, ChatActivity.class);
            if (type == ConversationType.single) {
                conv = JMessageClient.getSingleConversation(targetId, appKey);
                notificationIntent.putExtra(CApplication.TARGET_ID, targetId);
                notificationIntent.putExtra(CApplication.TARGET_APP_KEY, appKey);
                Log.d("Notification", "msg.fromAppKey() " + appKey);
            } else {
                conv = JMessageClient.getGroupConversation(Long.parseLong(targetId));
                notificationIntent.putExtra(CApplication.GROUP_ID, Long.parseLong(targetId));
            }
            notificationIntent.putExtra(CApplication.CONV_TITLE, conv.getTitle());
