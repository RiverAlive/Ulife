...
import butao.ulife.com.jpush.adapter.PickPictureAdapter;
...
public class PickPictureActivity extends BaseIMActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mGroupId = intent.getLongExtra(CApplication.GROUP_ID, 0);
        if (mGroupId != 0) {
            mIsGroup = true;
            mConv = JMessageClient.getGroupConversation(mGroupId);
        } else {
            mTargetId = intent.getStringExtra(CApplication.TARGET_ID);
            mTargetAppKey = intent.getStringExtra(CApplication.TARGET_APP_KEY);
    ...
    private OnItemClickListener onItemListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> viewAdapter, View view, int position, long id) {
            ...
            intent.putExtra(CApplication.POSITION, position);
            ...
            startActivityForResult(intent, CApplication.REQUEST_CODE_BROWSER_PICTURE);
        }
    };
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == CApplication.RESULT_CODE_SELECT_PICTURE) {
            if (data != null) {
        ...
        } else if (resultCode == CApplication.RESULT_CODE_BROWSER_PICTURE) {
            setResult(CApplication.RESULT_CODE_SELECT_ALBUM, data);
            finish();
        }
    }
            ...
            if (activity != null) {
                switch (msg.what) {
                    case SEND_PICTURE:
                        ...
                        activity.setResult(CApplication.RESULT_CODE_SELECT_ALBUM, intent);
