...
import butao.ulife.com.mvp.activity.DirverDetailActivity;
 ...
 * 功能描述 ：餐馆列表
 */
public class DinerLIstActivity extends MvpActivity<DinerColesPresenter> implements DinerColesView {
    @Bind(R.id.img_back)
    ImageView imgBack;
    @Bind(R.id.txt_title)
    TextView txtTitle;
    ...
    public void initView() {
        SinaRefreshView headerView = new SinaRefreshView(DinerLIstActivity.this);
        headerView.setArrowResource(R.mipmap.arrow);
        headerView.setTextColor(0xff745D5C);
        trlView.setHeaderView(headerView);
        LoadMoreView loadingView = new LoadMoreView(DinerLIstActivity.this);
    ...
    private void initRyView() {
        if (pagenum == 0) {
            LinearLayoutManager gridLayoutManager = new LinearLayoutManager(DinerLIstActivity.this);
            ryDiner.setLayoutManager(gridLayoutManager);
            commonRecycleAdapter = new CommonRecycleAdapter<DinerColesModel.DinerColes>(DinerLIstActivity.this, R.layout.adapter_dinercoles_item, dinerColesModel.getWares()) {
                @Override
...
//                        gotoActivity(DinerLIstActivity.this, DinerDetailActivity.class, bundle);
                        bundle.putString("StoreId", dinerColesModel.getWares().get(position).getId());
                        gotoActivity(DinerLIstActivity.this, ShopStoreActivity.class, bundle);
                    }
                }
            }
        }
