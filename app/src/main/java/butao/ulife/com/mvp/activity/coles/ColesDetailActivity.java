 ...
 * 功能描述 ：
 */
public class ColesDetailActivity extends MvpActivity<DinerDetailPresenter> implements DinerDetailView {
    ...
    public void initView() {
        SinaRefreshView headerView = new SinaRefreshView(ColesDetailActivity.this);
        headerView.setArrowResource(R.mipmap.arrow);
        headerView.setTextColor(0xff745D5C);
        trlView.setHeaderView(headerView);
        LoadMoreView loadingView = new LoadMoreView(ColesDetailActivity.this);
...
//                    ToastUtils.showShortToast("暂无更多数据");
            ...
            if (wareInfoModel != null) {
                if (wareInfoModel.getWareInfo() != null) {
                    ...
                    if (!TextUtils.isEmpty(wareInfoModel.getWareInfo().getPath())) {
                       Glide.with(ColesDetailActivity.this).load(wareInfoModel.getWareInfo().getPath()).into(imgMain);
                    }
                }
            }
        } else {
    ...
    private void initRYView() {
        if (pagenum == 0) {
            LinearLayoutManager gridLayoutManager = new LinearLayoutManager(ColesDetailActivity.this);
            ryMessage.setLayoutManager(gridLayoutManager);
            commonRecycleAdapter = new CommonRecycleAdapter<MessgaeModel.MessageItem>(ColesDetailActivity.this, R.layout.adapter_message, messgaeModel.getList()) {
                @Override
        ...
        switch (view.getId()) {
            ...
            case R.id.txt_map:
                if (wareInfoModel != null) {
                    if (wareInfoModel.getWareInfo() != null) {
                        if (!TextUtils.isEmpty(wareInfoModel.getWareInfo().getAddress())) {
                            LocationUtils.getLocation(ColesDetailActivity.this, wareInfoModel.getWareInfo().getAddress());
                        }
                    }
                }
            ...
            case R.id.txt_phone:
                if (wareInfoModel != null) {
                    if (wareInfoModel.getWareInfo() != null) {
                        if (!TextUtils.isEmpty(wareInfoModel.getWareInfo().getPhone())) {
                            PhoneDialog phoneDialog = new PhoneDialog(ColesDetailActivity.this, wareInfoModel.getWareInfo().getPhone());
                ...
                if (wareInfoModel != null) {
                    if (wareInfoModel.getWareInfo() != null) {
                        if (!TextUtils.isEmpty(wareInfoModel.getWareInfo().getId())) {
                            ...
                            gotoActivity(ColesDetailActivity.this, LeaveWordActivity.class, bundle);
                        }
                    }
                }
                break;
            case R.id.img_message:
                if (cApplication.isLogin()) {
                    if (!TextUtils.isEmpty(wareInfoModel.getWareInfo().getUid())) {
                        MessageChat messageChat = new MessageChat(ColesDetailActivity.this, wareInfoModel.getWareInfo().getUid());
                        messageChat.inModule();
                    } else {
                        ToastUtils.showShortToast("本店暂时不可私信");
                    }
                } else {
                    gotoActivity(ColesDetailActivity.this, LoginActivity.class);
                }
                break;
        }
    }
